...
Cross-Partner Spam Guard*
  ...
  **Attribute**                       **Value**
  ----------------------------------- ----------------------------------------------
  Document                            Leaex_WhatsApp_Architecture_v3.1

  Supersedes                          Leaex_WhatsApp_Integration_v2_Amendment.docx

  Architecture                        Rust (Axum + Tokio) + Redis + PostgreSQL +
                                     evo API
...
## 2.1 Authentication & Request Flow
  ...
  **Step**                **Action**                   **Failure Behaviour**
  ----------------------- ---------------------------- -----------------------
  1. Extract header      Read x-api-key or             401 Unauthorized
                          x-webhook-secret from header. immediately. No further
                                                        processing.

  2. Constant-Time Auth   Compare keys using           401 Unauthorized if
                          constant-time comparison      not a match. Prevents
                          to prevent timing attacks.    timing attacks.

  3. Tenant Resolve       Look up api_key in database.  403 Forbidden if not
                                                        found or tenant suspended.

  4. Tenant Validation    Confirm tenant belongs to     403 Forbidden.
...
## 14. Current Deployment State (v3.1 Runtime Truth)

This section reflects the **current code and compose configuration** in
`com/wa_api`.

### 14.1 Runtime Components

- `api-core` (Axum API gateway): auth, routing, webhook ingestion
- `worker-core` (Tokio worker): queue consumption + send pipeline
- `postgres` (single DB): wa_api schema (not Supabase in current deployment)
- `redis`: queues, locks, rate-limit and health cache
- `evo_api` (separate compose file): WhatsApp bridge layer

### 14.2 Environment File Strategy

  ---------------------------------------------------------------------------
  **File**          **Purpose**                      **Used By**
  ----------------- -------------------------------- -------------------------
  `.env.wa_api`     wa_api stack runtime and         `docker-compose.yml`,
                    compose variables                Rust services

  `env.evo`         evo API stack variables          `docker-compose.evo.yml`

  `env.leaex`       Leaex platform integration       Leaex app/service
                    variables for calling wa_api     configuration
  ---------------------------------------------------------------------------

### 14.3 Required Variables by Scope

#### A) wa_api required (`.env.wa_api`)
...
#### B) wa_api operational/recommended (`.env.wa_api`)
...
#### C) evo required (`env.evo`)

- `EVO_API_KEY` (must match wa_api `EVO_API_KEY`)
- `WA_API_WEBHOOK_URL` (base URL, evo appends `/webhook/evo`)
- `POSTGRES_PASSWORD` (if evo shares same postgres service credentials)

#### D) Leaex integration required (`env.leaex`)

- `WA_API_BASE_URL`
- `WA_API_X_API_KEY` (must match wa_api `PAUTH_API_KEY`)
- `WA_API_DEFAULT_TENANT_ID`
- `WA_API_ADMIN_KEY` (must match wa_api `ADMIN_API_KEY` for admin operations)

### 14.4 Critical Secret Alignment Rules

1. `EVO_API_KEY` must be identical in wa_api and evo config.
2. `WA_API_X_API_KEY` (Leaex) must equal wa_api `PAUTH_API_KEY`.
3. `WA_API_ADMIN_KEY` (Leaex) must equal wa_api `ADMIN_API_KEY`.
4. evo webhook calls to `/webhook/evo` must include
   `x-webhook-secret=<WEBHOOK_SHARED_SECRET>`.

### 14.5 Deployment Commands (recommended)

```bash
# wa_api stack
docker compose --env-file .env.wa_api up -d

# evo stack
docker compose -f docker-compose.evo.yml --env-file env.evo up -d
```

*End of Document \-- Leaex WhatsApp Architecture v3.1*
